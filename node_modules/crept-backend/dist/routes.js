...
const sign = (payload, refresh = false) => jwt.sign(payload, refresh ? (process.env.JWT_REFRESH_SECRET || 'refresh') : (process.env.JWT_SECRET || 'dev'), { expiresIn: refresh ? '30d' : '1d' });
...
apiRouter.patch('/users/:id', authRequired, (req, res) => {
    ...
    if (body.data.newPassword) {
        if (req.user.role !== 'admin' && (!body.data.currentPassword || !bcrypt.compareSync(body.data.currentPassword, cur.password_hash)))
            return res.status(400).json({ error: 'Current password invalid', code: 'PASSWORD_INVALID' });
...
apiRouter.patch('/licenses/:id', authRequired, adminRequired, (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: 'Invalid input', code: 'VALIDATION_ERROR' });
    const row = db.prepare('SELECT * FROM licenses WHERE id=?').get(Number(req.params.id));
    if (!row)
        return res.status(404).json({ error: 'Not found', code: 'NOT_FOUND' });
...
apiRouter.post('/hwid/reset-request', authRequired, (req, res) => {
    ...
    const last = db.prepare('SELECT requested_at FROM hwid_reset_requests WHERE user_id=? ORDER BY requested_at DESC LIMIT 1').get(req.user.id);
    if (last) {
        const diff = Date.now() - new Date(last.requested_at).getTime();
        if (diff < 30 * 86400000)
            return res.status(429).json({ error: 'Reset cooldown active', code: 'HWID_RESET_COOLDOWN' });
    }
...
apiRouter.post('/hwid/requests/:id/approve', authRequired, adminRequired, (req, res) => {
    const id = Number(req.params.id);
    const request = db.prepare('SELECT * FROM hwid_reset_requests WHERE id=?').get(id);
    if (!request)
        return res.status(404).json({ error: 'Not found', code: 'NOT_FOUND' });
    const license = db.prepare('SELECT * FROM licenses WHERE user_id=? ORDER BY id DESC LIMIT 1').get(request.user_id);
    if (license) {
...
apiRouter.get('/tickets/:id', authRequired, (req, res) => {
    ...
    if (!t)
        return res.status(404).json({ error: 'Not found', code: 'NOT_FOUND' });
    if (req.user.role !== 'admin' && req.user.id !== t.user_id)
        return res.status(403).json({ error: 'Forbidden', code: 'FORBIDDEN' });
...
apiRouter.post('/tickets/:id/reply', authRequired, (req, res) => {
    ...
    if (!body)
        return res.status(400).json({ error: 'Reply required', code: 'VALIDATION_ERROR' });
...
apiRouter.get('/stats/admin', authRequired, adminRequired, (_req, res) => {
    ...
    const ticketsOpen = db.prepare("SELECT COUNT(*) as c FROM tickets WHERE status IN ('Open','In Progress')").get().c;
    const mrr = 12940;
