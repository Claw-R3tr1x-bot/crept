...
function upsertUser(username: string, email: string, password: string, role: 'admin' | 'user') {
...
const users = db.prepare('SELECT id FROM users WHERE role = ?').all('user') as { id: number }[];
